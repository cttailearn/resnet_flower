...
def load_model():
    ...
    model.fc = nn.Linear(num_ftrs, 102)  # 类别数自己根据自己任务来

   
    checkpoint = torch.load('flower_resnet18.pth')
...
def prepare_image(image, target_size):
    ...
    if use_gpu:
        image = image.cuda()
    return image.requires_grad_(False) #不需要求导            
...
def predict():
    ...
    if flask.request.method == 'POST':
        ...
        if flask.request.files.get("image"):
            ...
            image = prepare_image(image, target_size=(96, 96))
            ...
            for prob, label in zip(results[0][0], results[1][0]):
                label_name = flower_to_name.get(str(label))
                r = {"label": str(label), "label_name": label_name, "probability": float(prob)}
                #将预测结果添加至data字典
